...
// Ajout BLA
void CScintillaView::OnFindNext(LPCTSTR lpszFind, bool bInvertDirection)
{
	ASSERT_VALID(this);
    _scintillaEditState.strFind = lpszFind; // pour que le repeat puisse marche
    bool bNext = _scintillaEditState.bNext;
    if (bInvertDirection)
    {
        bNext = !bNext;
    }
	if (!FindText(lpszFind, bNext, _scintillaEditState.bCase, _scintillaEditState.bWord, _scintillaEditState.bRegularExpression))
		TextNotFound(_scintillaEditState.strFind, _scintillaEditState.bNext, _scintillaEditState.bCase, _scintillaEditState.bWord, _scintillaEditState.bRegularExpression, FALSE);
}
// Fin ajout BLA
...
// Modif BLA
#ifdef EXE_ONGTV
IMPLEMENT_DYNAMIC(CScintillaDoc, COngTVDoc)
#else
