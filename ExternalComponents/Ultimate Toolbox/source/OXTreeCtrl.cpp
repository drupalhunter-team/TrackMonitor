...
// version modifiée par BLavier lors du passage à VS 2008 avec le MFC features pack.
// L'intégration du contrôle dans la classe CBrowseProcBar qui marchait avec UltimateToolBox
// ne marchait plus avec le MFC Features pack => les evts NM_RCLICK et NM_DBLCLK n'étaient plus
// remontées à la fenêtre CBrowseProcBar
// Les méthodes OnLButtonDblClk et OnRButtonDown ont donc été modifiées
...
// modif BLA
         if (NULL != m_pWndToNotify)
         {
            NMHDR nmh;
            nmh.code = NM_DBLCLK;    // Message type defined by control.
            nmh.idFrom = GetDlgCtrlID();
            nmh.hwndFrom = GetSafeHwnd();
            m_pWndToNotify->SendMessage(WM_NOTIFY, (WPARAM)GetSafeHwnd(), (LPARAM)&nmh);
         }
         else
         {
...
// modif BLA
         ...
         {
            NMHDR nmh;
            nmh.code = NM_RCLICK;    // Message type defined by control.
            nmh.idFrom = GetDlgCtrlID();
            nmh.hwndFrom = GetSafeHwnd();
            m_pWndToNotify->SendMessage(WM_NOTIFY, (WPARAM)GetSafeHwnd(), (LPARAM)&nmh);
         }
...
void COXTreeCtrl::SendSetDispInfoNotify(TV_ITEM* pTVI)
{
	...
	m_pWndToNotify->SendMessage(WM_NOTIFY,di.hdr.idFrom,(LPARAM)&di);
}
...
BOOL COXTreeCtrl::OnBeginlabeledit(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = m_pWndToNotify->SendMessage(TVN_BEGINLABELEDIT, pNMHDR->idFrom, (LPARAM) &tvdi);
	...
	if (GetParent() != NULL && m_pWndToNotify->SendMessage(WM_USER_QUERY_PROPERTIESWND))
		return FALSE;
	else
		return TRUE;
}
...
void COXTreeCtrl::SendDeleteItemNotify(TV_ITEM* ptvi)
{
	...
	m_pWndToNotify->SendMessage(WM_NOTIFY,nmt.hdr.idFrom , (LPARAM)&nmt);
}
...
BOOL COXTreeCtrl::SendItemExpandingNotify(HTREEITEM hItem,UINT nCode)
{
	...
	return (BOOL)(m_pWndToNotify->SendMessage(WM_NOTIFY,nmt.hdr.idFrom,(LPARAM)(&nmt)));
}
