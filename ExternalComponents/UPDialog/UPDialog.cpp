...
// Ajout BLA
    m_bOverWriteCancelButtonCaption = false;
// fin ajout
	...
	_tcsncpy_s(m_szDialogCaption,sizeof(m_szDialogCaption), lpszDlgTitle,(sizeof(m_szDialogCaption)/sizeof(m_szDialogCaption[0]))-1);
}

// Ajout BLA
CUPDialog::CUPDialog(HWND hParentWnd,LP_CUPDIALOG_USERPROC lpUserProc,LPVOID lpUserProcParam,LPCTSTR lpszDlgTitle/*=_T("Please Wait..")*/, LPCSTR lpszCancelButtonCaption /*= NULL*/)
{
	m_hThread = NULL;							//No Thread Yet !!
    ...
    if (lpszCancelButtonCaption)
    {
        m_bOverWriteCancelButtonCaption = true;
        m_bAllowCancel = true;
	    ZeroMemory(m_szCancelButtonCaption,sizeof(m_szCancelButtonCaption));
	    _tcsncpy_s(m_szCancelButtonCaption,sizeof(m_szCancelButtonCaption), lpszCancelButtonCaption,(sizeof(m_szCancelButtonCaption)/sizeof(m_szCancelButtonCaption[0]))-1);
    }
    else
    {
        m_bOverWriteCancelButtonCaption = false;
...
// Correction BLA : AfxGetResourceHandle() au lieu de NULL
	return DialogBoxParam(AfxGetResourceHandle(),MAKEINTRESOURCE(IDD),m_hParentWnd,ProgressDlgProc,(LPARAM)this);
}
...
// Ajout BLA
            if (pProgressDialog->m_bOverWriteCancelButtonCaption)
    			SendMessage(GetDlgItem(hDlg,CUPDIALOG_CANCELBUTTON_ID),WM_SETTEXT,0,(LPARAM)pProgressDialog->m_szCancelButtonCaption);
// fin ajout
...
// Ajout BLA
// Fonction qui fait une attente en affichant une progress bar
// Parametres :
//   lpszDlgTitle             Titre de la fenetre
//   dureeToWaitMs            Duree de l'attente
//   lpszCancelButtonCaption  Label du bouton Cancel => si NULL bouton Cancel pas autorise
//   stepWaitMs               Pas de l'attente (100 ms par defaut)
//
// Retour : IDOK    => l'attente est allee jusqu'a son terme
//         IDCANCEL => l'utilisateur a interrompu l'attente (appui sur le bouton cancel)
static bool StaticWait(const CUPDUPDATA* pCUPDUPData);
typedef struct {
    unsigned long dureeToWaitMs;
    unsigned long stepWaitMs;
} T_DATA_WAIT;
INT_PTR WaitWithProgressBar(LPCTSTR lpszDlgTitle, unsigned long dureeToWaitMs, LPCSTR lpszCancelButtonCaption /*= NULL*/, unsigned long stepWaitMs /*= 100*/)
{
    T_DATA_WAIT dataWait;
    dataWait.dureeToWaitMs = dureeToWaitMs;
    dataWait.stepWaitMs = stepWaitMs;
    CUPDialog Dlg(NULL, StaticWait, (LPVOID)&dataWait, lpszDlgTitle, lpszCancelButtonCaption);
    return Dlg.DoModal();
}

static bool StaticWait(const CUPDUPDATA* pCUPDUPData)
{
    T_DATA_WAIT *pDataWait = (T_DATA_WAIT*)pCUPDUPData->GetAppData();
    DWORD startWaitMs = GetTickCount();
    DWORD endWaitMs = startWaitMs + pDataWait->dureeToWaitMs;
    while (GetTickCount() < endWaitMs)
    {
        if (pCUPDUPData->ShouldTerminate())
        {
            return false;
        }
        Sleep(pDataWait->stepWaitMs);
        UINT percent =  (GetTickCount()-startWaitMs) * 100 / pDataWait->dureeToWaitMs;
        pCUPDUPData->SetProgress(_T(" "), percent);
    }
    return true;
}
