...
void CCheckComboBox::SelectAll(BOOL bCheck)
{
    ...
    // Pour utiliser dans une barre d'outil dérivant de CAdeMFCToolBarCheckComboBoxButton, on est obligé de forcer
    // le réaffichage comme suit pour que ça soit correctement affiche (je n'ai trouve que cette méthode ...)
    RecalcText();
	GetParent()->SendMessage(WM_COMMAND, MAKELONG(GetWindowLong(m_hWnd, GWL_ID), CBN_SELCHANGE), (LPARAM)m_hWnd);
}

// Are all checked ?
BOOL CCheckComboBox::AreAllChecked(void)
{
	INT nCount = GetCount();
